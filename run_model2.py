...
#         embedding_layer = EmbeddingLayer(200, 240, 'cnn', kernel_size=3)
    ...
    for epoch in range(7, num_epoch + 7):
        cumulative_loss = 0
...
#             target = Variable(torch.zeros(scores.size(0)).type(torch.LongTensor)) 
            ...
            if batch_idx % 20 == 1:
                print ('epoch:{}/{}, batch:{}/{}, loss:{}'.format(epoch, num_epoch+6, batch_idx, num_batch, loss.data[0]))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if eval: # lstm for now
            ...
            print ('Cumulative loss: {}'.format(cumulative_loss))
            print ('evaluating ....')
            do_eval(dev_map, dev_data, embedding_layer, 'Dev')
            print ('------------------')
            do_eval(test_map, test_data, embedding_layer, 'Test')
...
if __name__ == '__main__':
    ...
    restore_model(model, 'models/lstm_bi_epoch=5_margin=0.3_hidden=120_1')
    if cuda_available:
        model = model.cuda()
    train(model_option, model, batch_size=batch_size, num_epoch=epoch, margin=margin)

    print ('Saving Model...')
    save_model(model, 'models/'+str(model_option)+'_bi_epoch='+str(epoch)+'_margin='+str(margin)+'_hidden='+str(HIDDEN_DIM)+'_2')
    print ('Done')
